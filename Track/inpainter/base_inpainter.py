...
class BaseInpainter:
    def __init__(self, E2FGVI_checkpoint, device) -> None:
        ...
        self.device = device
        # load configurations
        with open("./Track/inpainter/config/config.yaml", "r") as stream:
    ...
    def inpaint_efficient(
        self, frames, masks, num_tcb, num_tca, dilate_radius=15, ratio=1
    ):
        """
        ...
        """
        assert (
            frames.shape[:3] == masks.shape
        ), "different size between frames and masks"
        ...
        masks = (
            torch.from_numpy(binary_masks).permute(0, 3, 1, 2).contiguous().unsqueeze(0)
        )
        ...
        for f in tqdm(
            range(0, video_length, self.neighbor_stride), desc="Inpainting image"
        ):
            neighbor_ids = [
                i
                for i in range(
            ...
            with torch.no_grad():
                ...
                for i in range(len(neighbor_ids)):
                    idx = neighbor_ids[i]
                    img = pred_imgs[i].astype(np.uint8) * binary_masks[idx] + frames[
                        idx
                    ] * (1 - binary_masks[idx])
    ...
    def inpaint(self, frames, masks, dilate_radius=15, ratio=1):
        ...
        id_splits = [
            [i * num_subset_frames, (i + 1) * num_subset_frames]
        ...
        for id_split in id_splits:
            ...
            try:
                tcb_frames = np.stack(
                    [
                        frames[idb]
                        for idb in range(
                            id_before, (id_split[0] - self.step) + 1, self.step
                        )
                    ],
                    0,
                )
                ...
                num_tcb = 0
            id_after = min(
                video_length, id_split[1] + self.step * self.num_external_ref + 1
            )
            try:
                tca_frames = np.stack(
                    [
                        frames[ida]
                        for ida in range(id_split[1] + self.step, id_after, self.step)
                    ],
                    0,
                )
                tca_masks = np.stack(
            ...
            inpainted_splits.append(
                self.inpaint_efficient(
                    video_split, mask_split, num_tcb, num_tca, dilate_radius, ratio
                )
            )
            torch.cuda.empty_cache()
...
def make_mask(video_state):
    image_size = (
        video_state["stabilize_images"][0].shape[0],
        video_state["stabilize_images"][0].shape[1],
    )
    # 压缩原视频(修改)
    ...
    # 计算黑色区域的大小     97%
    black_width = int(width * 0.97)
    black_height = int(height * 0.97)

    # 计算黑色区域的起始位置
    start_x = int((width - black_width) / 2)
    start_y = int((height - black_height) / 2)

    # 在图片上绘制黑色区域
    for x in range(start_x, start_x + black_width):
        for y in range(start_y, start_y + black_height):
            image.putpixel((x, y), (0, 0, 0))
    images = []

    for i in range(len(video_state["stabilize_images"])):
        images.append(image)

    video_state["masks"] = np.asarray(images)


def black_fix(video_state):
    # ----------------------------------------------
    # how to use
    # ----------------------------------------------
    # 1/3: set checkpoint and device
    checkpoint = "./Track/checkpoints/E2FGVI-HQ-CVPR22.pth"
    device = "cuda:0"
    ...
    make_mask(video_state)

    inpainted_frames = base_inpainter.inpaint(
        video_state["stabilize_images"], video_state["masks"]
    )  # numpy array, T, H, W, 3
    ...
    resize_frames = []
    frame_width = 1920
    frame_height = 1080  # 恢复视频尺寸
    for idx, frame in tqdm(enumerate(inpainted_frames)):
        new_frame = cv2.resize(
            frame, (frame_width, frame_height), interpolation=cv2.INTER_CUBIC
        )
        resize_frames.append(new_frame)

    return resize_frames


def black_fix_npy(frame_path, mask_path, output_path):
    ...
    # 	frame.save(os.path.join(output_folder, f'{ti:06d}.jpg'))

    torch.cuda.empty_cache()

    # 保存为.npy文件
    np.save(os.path.join(output_folder, "fix_frames.npy"), inpainted_frames)
...
if __name__ == "__main__":
    ...
    frame_path = "G:/PYNM/FIN/datasets/tmp_image/original_shake/original_shake.npy"
    mask_path = "G:/PYNM/FIN/datasets/image_mask/original_shake/masks.npy"
    output_path = "G:/PYNM/FIN/datasets/fix_image/"
    black_fix(frame_path, mask_path, output_path)
